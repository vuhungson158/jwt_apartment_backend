...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.csrf().disable()
                ...
                .authorizeRequests().antMatchers("/", "/register", "/active", "/login").permitAll()
                .and().authorizeRequests().antMatchers("/getUser").hasRole(User.USER)
                .and().authorizeRequests().anyRequest().authenticated()
                .and().httpBasic().authenticationEntryPoint((request, response, authException)
                -> util.writeResponse(response, 401, "Phiên đăng nhập hết hạn."))
                .and().exceptionHandling().accessDeniedHandler((request, response, accessDeniedException)
                -> util.writeResponse(response, 403, "Không đủ quyền."));
    }
}